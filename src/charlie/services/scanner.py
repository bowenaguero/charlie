...
def _match_task(
    ...
    match target_type:
        case ComponentType.PLAYBOOK:
            return _match_playbook_task(file_path, task_id, task_def, task_type, target, source_name)
        case ComponentType.AUTOMATION:
            return _match_automation_task(file_path, task_id, task_def, task_type, target, source_name)
...
def _match_field_task(
    ...
    if is_setter and target in script_args:
        refs.append(
            _make_ref(
                file_path,
                source_name,
